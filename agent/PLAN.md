...
- [ ] Tuning: độ khó fair; đảm bảo **60 FPS** — 🔶 ĐANG TEST (2026-08-12): FPS counter `Tools/Void Runner/Add FPS Counter (Open Scene)` — **xuyên scene (DontDestroyOnLoad, chống trùng)** nên gắn 1 lần ở MainMenu là thấy cả Game/GameOver; hiện FPS/ms/GC trên màn hình (tắt bằng F3) + **log `[FPS-LOG]` mỗi 10s ra Console** (FPS/ms/GC/ten scene); hướng dẫn đầy đủ `agent/TESTING.md`; **FPS thật phải đo trên Chrome sau khi build WebGL** (Editor nhanh hơn build thật); test nhiều độ phân giải
