...
| Editor tools (`_Project/Editor/`) | Setup scene/UI/skybox tự động — **11 file idempotent** (cleanup 2026-08-15: xóa 4 tool one-shot RefactorGameplayTool/RenameGameTitleTool/SpriteBatchConverter/KenneyFontImporter + FPSInjectTool đã xóa từ trước) | Tự viết (menu `Tools/Starveil Runner/` — gom 3 nhóm **Setup/ · Optimize/ · Fix/**): Setup: UIOverhaulTool, SkyboxSetupTool, VFXSetupTool, MaterialLightingSetupTool, PostProcessingSetupTool, ShipSelectSetupTool, EnemyMonsterSetupTool, SciFiObstacleSetupTool · Optimize: BuildOptimizerTool · Fix: AlwaysIncludedShadersTool (2026-08-15) + helper UIBuilderHelpers |
