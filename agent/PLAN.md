...
- [x] VFX ambient (v3f.6 → v3f.7.2, 2026-08-12): **hệ sao trôi SpaceDrift** quanh player (chấm tròn mềm — chuyển sang shader `Sprites/Default` vì URP ParticlesUnlit mặc định OPAQUE → texture tròn thành khối vuông; xem CHANGELOG) + **lửa tên lửa** đuôi tàu (chùm cam mảnh, shape Cone 6° không văng lung tung) + **bỏ hẳn vệt trail tím** (root cause: `Particles/Additive` KHÔNG tồn tại trong URP → `new Material(null)` = error shader tím — R3.1) + dọn DIAG logs — ⏳ **chờ user test xác nhận v3f.7.2** (sao tròn + lửa mảnh)
