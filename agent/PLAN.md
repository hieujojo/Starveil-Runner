...
- [x] VFX ambient (v3f.6 → v3f.8, 2026-08-12): **hệ sao trôi SpaceDrift** quanh player (chấm tròn mềm — shader `Sprites/Default`, vì URP ParticlesUnlit mặc định OPAQUE → khối vuông; xem CHANGELOG) + **lửa tên lửa** đuôi tàu (v3f.8: BỎ cube "Thruster" vuông — root cause "ô vuông cam nối nhau" — thay bằng **Light cam lập lòe + hạt tròn nở rộng/mờ dần hình lưỡi lửa**, self-heal dựng lại nếu còn material cũ) + **bỏ hẳn vệt trail tím** (root cause: `Particles/Additive` KHÔNG tồn tại trong URP → error shader tím — R3.1) — ⏳ **chờ user test xác nhận v3f.8** (lửa = chùm tròn mềm, hết ô vuông) — kèm DIAG log `[DIAG-FLAME]` để xác nhận material
