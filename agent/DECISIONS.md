...
- **R0.10 — Road width (roadHalfWidth) là hằng số ĐỒNG BỘ TOÀN CỤC — sửa phải quét MỌI chỗ hardcode:** `Tile.roadHalfWidth`, scene `Ground scale x`, `laneWidth` (Player/Obstacle/Pickup). *(Editor tool `RefactorGameplayTool` — ĐÃ XÓA 2026-08-15.)* *(AmbientScroller đã xóa 2026-08-11 — không còn trong danh sách đồng bộ.)* Bỏ sót 1 chỗ (đặc biệt tool Editor hardcode giá trị CŨ) → chạy lại tool "phá" road mới, props đè road tái phát. *(Bug vòng 7 2026-08-11: road 14 → 18.)*
...
- **R0b.1 — Skybox có thể đổi theo scene/mood:** Game scene dùng Nebula (sâu, tinh vân), MainMenu dùng SpaceSkies Purple (nhẹ, tông hư không). Tool `SkyboxSetupTool` đã hỗ trợ cả 2 (menu `Tools/Starveil Runner/Setup/Skybox ...`).
