...
- **R3.14** — **Khi xóa 1 script (class), PHẢI quét Editor tools tham chiếu nó TRƯỚC** (`grep -rln '<TênClass>' Assets/_Project/Editor/`) — tool tham chiếu class đã xóa = **CS0246 compile fail toàn project**. Nếu tool chỉ phục vụ đúng thứ đã xóa → xóa luôn tool. *(Bug 2026-08-11: xóa AmbientScroller.cs → AmbientSetupTool vỡ.)*
