...
- **R7.25** — **KHÔNG thêm footer `🤖 Generated with Codebuff` / `Co-Authored-By: Codebuff` vào commit message** — tạo contributor "Codebuff" giả trên GitHub (đếm từ email trong commit). Commit chỉ gồm subject + body thuần; AI commit thay user không ghi tên tác giả khác. *(Quyết định 2026-08-15 — user yêu cầu bỏ; 19 commit cũ đã rewrite sạch bằng `git filter-branch` + backup tag `backup-before-history-rewrite`. Xem REFERENCE.md PART 4 quy tắc 6 + CHANGELOG.)*
